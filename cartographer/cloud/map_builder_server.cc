...
void MapBuilderServer::ProcessLocalSlamData(sensor::TimedPointCloudData& time_point_cloud)
{
    ...
    sensor_data->pose_data=transform::Rigid3d(pose_estimate.translation(),pose_estimate.rotation());
    sensor_data->data.time=time_point_cloud.time;
    sensor_data->data.origin=time_point_cloud.origin;
  //  sensor_data->data.ranges.resize(time_point_cloud.ranges.size());
    int count=0;
     for (const TimedRangefinderPoint& range : location_->time_point_cloud_filter.ranges)
     {
        // if(count<100)
         {
            sensor_data->data.ranges.push_back(range);
         }
         count++;
     }
...
//    sensor_data->data.set_allocated_origin(data_proto.mutable_origin());
 ...
 //     }
       LOG(INFO) << "Enqueue data;size:"<<sensor_data->data.ranges.size();
