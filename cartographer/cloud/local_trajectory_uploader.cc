...
constexpr int kConnectionRecoveryTimeoutInSeconds = 10;
...
void LocalTrajectoryUploader::ProcessSendQueue() {
  ...
  while (!shutting_down_) {
    ...
    if (sensor_data) {
            ...
            transform::proto::Rigid3d pose_data=cartographer::transform::ToProto(sensor_data->pose_data);
        //   sensor::proto::TimedPointCloudData data_proto=cartographer::sensor::ToProto(*sensor_data);
            request.mutable_sensor_metadata()->set_client_id("client_1");
            request.mutable_sensor_metadata()->set_sensor_id("sensor_1");
           request.mutable_local_pose()->set_allocated_rotation(pose_data.mutable_rotation());//= pose_data;
           request.mutable_local_pose()->set_allocated_translation(pose_data.mutable_translation());
           request.mutable_timed_point_cloud_data()->set_timestamp(common::ToUniversal(sensor_data->data.time));
       // request.mutable_timed_point_cloud_data()->set_allocated_origin(cartographer::transform::ToProto(sensor_data->data.origin));
         //  request.mutable_timed_point_cloud_data()->mutable_point_data()->Reserve(sensor_data->data.ranges.size());
        //    LOG(INFO) <<"pose_data_szie: "<<sensor_data->data.ranges.size();
           for (const cartographer::sensor::TimedRangefinderPoint& range : sensor_data->data.ranges) {
            //   *sensor_data->data.add_point_data() = ToProto(range);
              *request.mutable_timed_point_cloud_data()->add_point_data()=ToProto(range);
   ...
   //             common::FromSeconds(5), kUnrecoverableStatusCodes));
      //  async_grpc::Client<handlers::AddRangefinderDataSignature> client(
      //      client_channel_, common::FromSeconds(kConnectionTimeoutInSeconds));
...
try {
    ...
    {
        LOG(INFO) << "Write data fail!! :size:"<<request.ByteSize();
    }
