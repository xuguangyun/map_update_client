...
void Laserlocation::ProcessLaserdata(sensor::TimedPointCloudData& time_point_cloud)
{
    ...
    sensor::PointCloud point_cloud_;
     ...
     time_point_cloud_filter.ranges.clear();
     for(int i=0;i<point_cloud_filter.size();i++)
     {
         time_point_cloud_filter.ranges.push_back({point_cloud_filter[i].position,0});
     }
     if(!location_ok)
     {
     ...
     LOG(INFO) <<"ceres_now:"<<transform::ToProto(pose_observation).DebugString()<<std::endl;
     }
